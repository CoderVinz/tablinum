...
| `/wiki-mode` (v1.8) | Methodology modes (Engineering / LYT / PARA / Zettelkasten / Generic). Set via `bash bin/setup-mode.sh`; consumed by wiki-ingest / save / autoresearch for routing new pages |
...
Pick an organizational style for the vault via `bash bin/setup-mode.sh`. Five modes available: **engineering** (tablinum's own layout — concrete top-level folders projects/operations/entities/sources/concepts/resources; the shipped default, authoritative table in `wiki/meta/engineering-conventions.md`), **generic** (v1.7 default — no opinion), **LYT** (Linking Your Thinking — MOCs + atomic notes), **PARA** (vanilla Projects/Areas/Resources/Archives), **Zettelkasten** (timestamped IDs, flat, dense linking). The mode is written to `.vault-meta/mode.json` (committed for tablinum). `wiki-ingest`, `save`, and `autoresearch` consult `python3 scripts/wiki-mode.py route <type> "<name>"` before filing new pages — no special-casing needed in the consumer skills. Full guide: [docs/methodology-modes-guide.md](docs/methodology-modes-guide.md). Closes priority gap 5 from the May 2026 compass artifact.
